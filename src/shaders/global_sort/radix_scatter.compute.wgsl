...
fn key_digit(key: u32, shift: u32) -> u32 {
    return (key >> shift) & 255u;
}

fn count_bits_before(
    bin: u32,
    word_id: u32,
    bit_mask: u32,
) -> u32 {
    ...
    for (var i = 0u; i < FLAG_WORDS; i = i + 1u) {
        ...
        if i < word_id {
            prefix += countOneBits(bits);
        }

        if i == word_id {
            prefix += countOneBits(bits & (bit_mask - 1u));
        }
    }
...
fn main(
    ...
    let histogram_base = lid * params.num_workgroups;
    ...
    for (var j = 0u; j < params.num_workgroups; j = j + 1u) {
        ...
        total_count += histograms[histogram_base + j];
    }
    ...
    loop {
        if scan_offset >= RADIX_SORT_BINS {
            break;
        }

        let index = (lid + 1u) * (scan_offset << 1u) - 1u;
        if index < RADIX_SORT_BINS {
            bin_scan[index] += bin_scan[index - scan_offset];
        }
    ...
    loop {
        let index = (lid + 1u) * (scan_offset << 1u) - 1u;
        if index < RADIX_SORT_BINS {
            let temporary = bin_scan[index - scan_offset];

            bin_scan[index - scan_offset] = bin_scan[index];
    ...
    for (var block = 0u; block < params.num_blocks_per_workgroup; block = block + 1u) {
        ...
        if valid {
            ...
            for (var i = 0u; i < FLAG_WORDS; i = i + 1u) {
                bin_count_in_block += countOneBits(atomicLoad(&bin_flags[bin].words[i]));
            }
        ...
        if valid {
            if local_prefix == bin_count_in_block - 1u {
                global_offsets[bin] += bin_count_in_block;
            }
        }
